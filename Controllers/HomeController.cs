...
private readonly string _bucketUrl = "https://tu-proyecto.supabase.co/storage/v1/object/public/";
...
public async Task<IActionResult> Index(string busqueda)
{
    ...
    {
        // Traemos las reseñas, si falla o no hay, mandamos lista vacía
...
public async Task<IActionResult> PaginaCliente(string nombreUrl)
{
    ...
    if (empresa == null) return NotFound("Negocio no encontrado.");

    // 1. SERVICIOS
    ViewBag.Servicios = await _context.servicios
        .Where(s => s.id_empresa == empresa.id_empresa && s.activo)
        .ToListAsync() ?? new List<Servicio>();

    // 2. PERSONAL (FIX CON DYNAMIC)
    var listaPersonal = await (from s in _context.secretarios
                               join u in _context.usuarios on s.id_usuario equals u.id_usuario
                               where s.id_empresa == empresa.id_empresa && s.subrol == "prestador"
                               select new { u.nombre, s.id_secretario }).ToListAsync();
    
    // Al castear a dynamic, el operador ? : ya no se queja
    ViewBag.Personal = listaPersonal.Any() ? (dynamic)listaPersonal : new List<object>();

    // 3. RESEÑAS (FIX CON DYNAMIC)
    var listaResenas = await (from r in _context.resenas
                              join u in _context.usuarios on r.id_usuario equals u.id_usuario
    ...
    ViewBag.Resenas = listaResenas.Any() ? (dynamic)listaResenas : new List<object>();

    // 4. GALERÍA
    ViewBag.Galeria = await _context.galeria_fotos
        .Where(g => g.id_empresa == empresa.id_empresa)
        .ToListAsync() ?? new List<GaleriaFoto>();

    string nombreVista = empresa.id_plantilla == 2 ? "Plantilla_2" : "Plantilla_1";
...
public async Task<IActionResult> SubirFotoGaleria(IFormFile archivoGaleria, int idEmpresa)
{
    if (archivoGaleria != null && archivoGaleria.Length > 0)
    {
        ...
        // 2. Creamos el registro en la tabla de fotos
...
public async Task<IActionResult> EditarNegocio(Empresa e, IFormFile logoArchivo, IFormFile bannerArchivo)
{
    ...
    empresaDb.descripcion = e.descripcion; // <--- ¡AQUÍ SE GUARDA EL "ACERCA DE"!
    ...
    return RedirectToAction("Admin"); // Te regresa al panel después de guardar
}
[HttpPost]
public async Task<IActionResult> ProcesarRegistroNegocio(Empresa e, string rubroElegido, string rubroOtro, IFormFile logoArchivo)
{
    ...
    var empresaExistente = await _context.empresas.AnyAsync(x => x.id_administrador == userId);
    if (empresaExistente) 
    {
        TempData["Error"] = "Ya tienes un negocio registrado, padrino.";
        return RedirectToAction("Admin");
    }
    ...
    try 
    {
        ...
        // Actualizamos el rol en la sesión para que el Dashboard lo deje pasar
...
// --- VISTA: PERFIL Y CITAS DEL USUARIO ---
[Route("mi-perfil")]
public async Task<IActionResult> MiPerfil()
{
    ...
    // Obtenemos sus citas con el nombre del negocio
    ViewBag.MisCitas = await (from c in _context.citas
                             ...
                             select new { 
                                 c.id_cita, c.fecha, c.hora, c.estatus, e.nombre_empresa 
                             }).ToListAsync();
...
[HttpPost]
public async Task<IActionResult> CancelarCitaUsuario(int idCita)
{
    var cita = await _context.citas.FindAsync(idCita);
    if (cita != null && cita.estatus.ToLower() == "pendiente") 
    {
...
// --- ACCIÓN: ACTUALIZAR FOTO Y DATOS ---
[HttpPost]
public async Task<IActionResult> GuardarPerfil(Usuario u, IFormFile fotoPerfil)
{
    var userId = HttpContext.Session.GetInt32("UsuarioId");
    var userDb = await _context.usuarios.FindAsync(userId);

    if (userDb != null) {
        userDb.nombre = u.nombre;
        userDb.correo = u.correo;

        if (fotoPerfil != null && fotoPerfil.Length > 0) {
            // Guardamos la foto en el bucket/carpeta local
            userDb.foto_url = await GuardarFile(fotoPerfil, "perfiles");
            HttpContext.Session.SetString("UsuarioFoto", userDb.foto_url);
        }
...
public async Task<IActionResult> GuardarHorarios(int id_empresa, IFormCollection form)
{
    ...
    for (int i = 0; i < 7; i++)
    {
        string aperturaStr = form[$"apertura_{i}"].ToString();
        string cierreStr = form[$"cierre_{i}"].ToString();
        
        // Intentamos parsear, si falla usamos 00:00
        TimeSpan.TryParse(aperturaStr, out TimeSpan apertura);
        TimeSpan.TryParse(cierreStr, out TimeSpan cierre);
...
//agendar cita
[HttpPost]
public async Task<IActionResult> AgendarCita(Cita nuevaCita, string telefonoCliente)
{
    // 1. Validamos la sesión de forma segura
    int? usuarioId = HttpContext.Session.GetInt32("UsuarioId");
    
    if (usuarioId == null) 
    {
        return Json(new { success = false, mensaje = "Sesión expirada. Inicia sesión de nuevo." });
    }

    int idActual = usuarioId.Value;

    try 
    {
        // 2. Registro en tabla clientes (CRM)
        var cliente = await _context.clientes.FindAsync(idActual);
        if (cliente == null)
        {
            _context.clientes.Add(new Cliente { 
                id_usuario = idActual, 
                numero_telefono = telefonoCliente 
            });
            await _context.SaveChangesAsync();
        }

        // 3. Lógica de Expediente (Folio único)
        var expediente = await _context.expedientes
            .FirstOrDefaultAsync(e => e.id_cliente == idActual);

        if (expediente == null)
        {
            var nuevoExp = new Expediente {
                id_cliente = idActual,
                folio = "AXC-" + DateTime.Now.Ticks.ToString().Substring(10),
        ...
        // 4. Guardar Cita vinculada al trámite
        nuevaCita.id_usuario_tramito = idActual;
...
public async Task<IActionResult> AdminDashboard()
{
    ...
    // Admin y Secretario pueden entrar
    if (userId == null || (rol != "admin" && rol != "secretario")) 
        return RedirectToAction("login");

    var empresa = await _context.empresas.FirstOrDefaultAsync(e => e.id_administrador == userId || 
                  _context.secretarios.Any(s => s.id_usuario == userId && s.id_empresa == e.id_empresa));

    if (empresa == null) return RedirectToAction("Index");

    // CRM: Citas y Expedientes detallados
    ViewBag.Citas = await _context.citas.Where(c => c.id_empresa == empresa.id_empresa).ToListAsync();
    ViewBag.Expedientes = await _context.expedientes
        .Where(ex => _context.citas.Any(c => c.id_empresa == empresa.id_empresa && c.id_expediente == ex.id_expediente))
        .ToListAsync();
...
            return $"/{folder}/{name}";
        }
// --- ACTUALIZAR CONFIGURACIÓN (LOGO, BANNER, GPS) ---
// --- ACTUALIZAR CONFIGURACIÓN (LOGO, BANNER, GPS) ---
[HttpPost]
public async Task<IActionResult> ActualizarConfiguracion(int idEmpresa, string? bannerUrl, string? logoUrl, double lat, double lng)
{
    var emp = await _context.empresas.FindAsync(idEmpresa);
    if (emp == null) return Json(new { success = false, mensaje = "No se encontró el negocio." });

    try {
        if (!string.IsNullOrEmpty(bannerUrl)) emp.banner_url = bannerUrl;
        if (!string.IsNullOrEmpty(logoUrl)) emp.logotipo_url = logoUrl;
        
        emp.ubicacion_lat = lat;
        emp.ubicacion_lng = lng;

        await _context.SaveChangesAsync(); // Persistencia en PostgreSQL
        return Json(new { success = true, mensaje = "¡Negocio actualizado!" });
    }
    catch (Exception ex) {
...
// --- MÉTODO PARA MAPEAR LA URL DEL BUCKET ---
[HttpPost]
public async Task<IActionResult> ConfirmarSubida(string nombreArchivo, string carpeta)
{
    // Reemplaza con tu URL real de Supabase
    string supabaseUrl = "https://tu-proyecto.supabase.co/storage/v1/object/public/";
    string urlFinal = $"{supabaseUrl}{carpeta}/{nombreArchivo}";

    return Ok(new { url = urlFinal });
}

// --- SUBIDA DE ARCHIVOS A BUCKETS ---
[HttpPost]
public async Task<IActionResult> SubirImagen(IFormFile archivo, string carpeta)
{
    if (archivo == null || archivo.Length == 0) return BadRequest("Archivo vacío.");

    // Generamos un nombre único para evitar duplicados en Supabase
    string nombreArchivo = Guid.NewGuid().ToString() + Path.GetExtension(archivo.FileName);
    
    // Aquí el front-end usará la URL que le devolvemos para guardarla en la DB
    string urlFinal = $"{_bucketUrl}{carpeta}/{nombreArchivo}";

    // NOTA: Para la demo, el Front enviará el archivo a Supabase y este método 
    // solo confirma la ruta para mapearla en PostgreSQL
    return Ok(new { url = urlFinal });
}    
}
    }