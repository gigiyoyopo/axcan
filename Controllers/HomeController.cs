...
public async Task<IActionResult> PanelSecretario()
{
    ...
    return View("Secretario", empresa); // Aquí usas el HTML de secretario.html convertido a .cshtml
}
        [HttpGet]
...
ViewBag.EsStaff = await _context.secretarios.AnyAsync(s => s.id_usuario == userId);
        ...
        public async Task<IActionResult> ProcesarRegistro(Usuario u)
        {
            try 
            {
                ...
                TempData["Mensaje"] = "¡Bienvenido a AXCAN! Tu cuenta ha sido creada.";
...
public async Task<IActionResult> Admin()
{
    ...
    // 2. ¿Es un Empleado (Secretario o Prestador)?
    var staff = await _context.secretarios.FirstOrDefaultAsync(s => s.id_usuario == userId);
    if (staff != null) 
    {
    ...
    // 3. Si no es nada de lo anterior, a registrar negocio
...
public async Task<IActionResult> AgendarCita(Cita nuevaCita)
{
    ...
    try 
    {
        // 1. EL TRUCO MAESTRO: Verificar si existe en la tabla CLIENTES
        // Tu SQL obliga a que id_cliente exista en public.clientes
        var esCliente = await _context.clientes.AnyAsync(c => c.id_usuario == userId);
        
        if (!esCliente)
        {
            // Si no existe, lo insertamos en la tabla clientes primero
            // El SQL pide id_usuario y numero_telefono (le ponemos uno por defecto)
            var nuevoCliente = new Cliente {
                id_usuario = userId.Value,
                numero_telefono = "0000000000" // Se actualizará después
            };
            _context.clientes.Add(nuevoCliente);
            await _context.SaveChangesAsync(); 
        }

        // 2. Ahora sí manejamos el Expediente
...
}// 1. Vista del Panel con datos reales[HttpGet]
public async Task<IActionResult> Secretario()
{
    var userId = HttpContext.Session.GetInt32("UsuarioId");
    if (userId == null) return RedirectToAction("login");

    // Usamos un try-catch para atrapar el error de cast si la BD tiene datos corruptos
    try 
    {
    ...
    {
        // Si sale el error de cast, te mandará al Index en lugar de mostrar la pantalla blanca
        Console.WriteLine("Error de datos en staff: " + ex.Message);
        return RedirectToAction("Index");
    }
}
// 2. Acción para que el secretario cambie el estado de la cita (AJAX)
[HttpPost]
public async Task<IActionResult> ActualizarEstatusCita(int id_cita, string nuevoEstado)
{
    try {
        ...
        cita.estatus = nuevoEstado; // 'completada' o 'cancelada'
...
{
    // Si necesitas pasar datos de Koi Labs, hazlo aquí
    return View(); 
}
       [HttpPost]
public async Task<IActionResult> SubirFotoGaleria(IFormFile archivoGaleria, int idEmpresa)
{
    if (archivoGaleria != null && archivoGaleria.Length > 0)
    {
        ...
        // Guardamos el registro en la tabla galeria_fotos
    ...
    // Redirigimos al admin para que el Iframe se actualice
    return RedirectToAction("Admin");
}
 [HttpPost]
...
public async Task<IActionResult> GetDatosGraficas(int idEmpresa)
{
    ...
    // Inicializar arreglo de 7 días (Lunes a Domingo)
